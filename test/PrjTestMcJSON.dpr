...
begin
  ...
  try
    ...
    S := McJsonEscapeString('\a"b"ч');
    Result := Result and (S = '\\a\"b\"\u00E7');
    // unescape function
    S := McJsonUnEscapeString('aB\t\n\u00e7d\u00e7'); // debug sees 'ч' in S.
    Result := Result and (S = 'aB' + #9 + #10 + 'чdч');
...
begin
  ...
  try
    ...
    for i := 1 to 2 do
      N['array'].Add.AsJSON := '{"k'+IntToStr(i)+'": "ч'+IntToStr(i)+'"}';
...
begin
  ...
  try
    try
      ...
      // alternative (fail: it changes id 1Дs value from boolean to string)
